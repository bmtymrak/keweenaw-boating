...
def get_buoy_data(url, name):
    ...
    time_utc = datetime.datetime.strptime(f'{data[2][1]}/{data[2][2]}/{data[2][0]} {data[2][3]}:{data[2][4]}', '%m/%d/%Y %H:%M')
    ...
    info = {
            ...
            f'{name}_wind_speed': round(float(data[2][6])/.44704, 2),
            f'{name}_wind_gusts': round(float(data[2][7])/.44704, 2),
...
def home_page_view(request):
    ...
    north_data = get_buoy_data('https://www.ndbc.noaa.gov/data/realtime2/45023.txt', 'north')
    south_data = get_buoy_data('https://www.ndbc.noaa.gov/data/realtime2/45025.txt', 'south')
